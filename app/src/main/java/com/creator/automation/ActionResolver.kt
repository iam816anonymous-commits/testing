...
class ActionResolver {
    ...
    /**
     * Resolves a target against a ScreenObservation using fused elements (combining Accessibility, OCR, and Vision).
     */
    fun resolveFusedTarget(observation: ScreenObservation, target: String): TargetResolutionResult {
        if (target.isBlank()) {
            return TargetResolutionResult(
                match = null,
                candidateCount = 0,
        ...
        // 1. First try primary UiSnapshot if accessibility nodes are available
        if (observation.accessibilityNodes.isNotEmpty()) {
            val snapshot = UiSnapshot(
                packageName = observation.packageName,
                allNodes = observation.accessibilityNodes
            )
            val snapshotResult = resolveTargetWithAmbiguity(snapshot, trimmedTarget)
            if (snapshotResult.status == TargetResolutionStatus.FOUND_UNIQUE) {
                return snapshotResult
            }
        }

        // 2. Fallback to fused visual/OCR elements
        val matches = observation.fusedElements.filter { elem ->
            (elem.text != null && elem.text.contains(trimmedTarget, ignoreCase = true)) ||
            (elem.contentDescription != null && elem.contentDescription.contains(trimmedTarget, ignoreCase = true)) ||
            (elem.viewId != null && elem.viewId.contains(trimmedTarget, ignoreCase = true))
        }

        if (matches.isNotEmpty()) {
            val isAmbiguous = matches.size > 1
            val best = matches.first()
            val syntheticNode = best.nodeRef as? UiNodeInfo ?: UiNodeInfo(
                text = best.text,
                contentDescription = best.contentDescription,
                viewIdResourceName = best.viewId,
                boundsInScreen = "${best.bounds.left}, ${best.bounds.top}, ${best.bounds.right}, ${best.bounds.bottom}",
                isClickable = true,
            ...
            return TargetResolutionResult(
                match = ResolutionMatch(
                    node = syntheticNode,
                    matchMethod = "FUSED_PERCEPTION_${best.sources.joinToString("_")}",
                    confidence = if (isAmbiguous) 0.55 else best.confidence,
                    reason = "Matched fused element text/description containing '$trimmedTarget'"
                ),
                ...
                explanation = if (isAmbiguous) "Multiple fused elements matched '$target'" else "Uniquely resolved fused element '$target'"
            )
        }
        ...
        return TargetResolutionResult(
            ...
            isAmbiguous = false,
            status = TargetResolutionStatus.NOT_FOUND,
            explanation = "Target '$target' not found in fused screen observation"
        )
    }
