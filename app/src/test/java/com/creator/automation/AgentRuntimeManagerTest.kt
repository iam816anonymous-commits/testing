...
class AgentRuntimeManagerTest {
    ...
    @Test
    fun testCancelActiveSession_IsIdempotent() = runBlocking {
        val manager = AgentRuntimeManager(
            context = mockContext,
            sessionDao = mockSessionDao,
            auditDao = mockAuditDao,
        ...
        manager.startOrResumeTaskSession("Find analytics")
        val firstCancel = manager.cancelActiveSession("User cancelled once")
        val secondCancel = manager.cancelActiveSession("User cancelled twice")
    ...
    @Test
    fun testStartNewTask_PostCancellation_ExecutesCleanly() = runBlocking {
        val manager = AgentRuntimeManager(
            context = mockContext,
            sessionDao = mockSessionDao,
            auditDao = mockAuditDao,
        ...
        Mockito.`when`(mockSessionDao.getActiveSessionByTaskDescription("Task 1")).thenReturn(null)
        Mockito.`when`(mockSessionDao.getActiveSessionByTaskDescription("Task 2")).thenReturn(null)
        ...
        Mockito.`when`(mockAgentCore.executeTaskStep(
            ...
            AgentStepResult(
                stateBefore = AgentState.IDLE,
                observation = null,
                decisionReason = "Task 2 completed",
                actionExecuted = null,
                actionResult = null,
                verificationStatus = VerificationStatus.SUCCESSFULLY_VERIFIED,
                nextState = AgentState.COMPLETED
        ...
        // Start Task 1 & Cancel
        manager.startOrResumeTaskSession("Task 1")
        manager.cancelActiveSession("Cancelled Task 1")

        // Start Task 2 post-cancellation
        val stepResult2 = manager.startOrResumeTaskSession("Task 2")

        assertEquals(AgentState.COMPLETED, stepResult2.nextState)
        assertEquals("Task 2", AgentRuntimeManager.activeSession.value?.taskDescription)
        Mockito.verify(mockAgentCore, Mockito.atLeastOnce()).resumeAgent()
    }
}
